...
object Huffman {
  ...
  def weight(tree: CodeTree): Int = tree match {
    case Leaf(_,_) => tree.asInstanceOf[Leaf].weight 
    case Fork(_,_,_,_) => weight(tree.asInstanceOf[Fork].left) + 
    		weight(tree.asInstanceOf[Fork].right)
  }

  def chars(tree: CodeTree): List[Char] = tree match {
    case Leaf(_,_) => List(tree.asInstanceOf[Leaf].char)
    case Fork(_,_,_,_) => chars(tree.asInstanceOf[Fork].left) :::
    	chars(tree.asInstanceOf[Fork].right)
  }
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def accBuild(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = {
      if (chars.isEmpty) acc
      else {
        val c = chars.head
        accBuild(chars.tail, updateCount(c, acc))
      }
    }
    
    def updateCount(c: Char, acc: List[(Char, Int)]): List[(Char, Int)] = {
      if (acc.isEmpty) List((c, 1))
      else {
        val pair = acc.head
        if (c == pair._1) {
          val newPair = (pair._1 , pair._2  + 1)
          newPair :: acc.tail
        }
        acc.head :: updateCount(c, acc.tail)
      }
    }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    ...
    def makeList(xs: List[(Char, Int)]): List[Leaf] = {
      if (xs.isEmpty) List()
      else {
        val pair = xs.head
        Leaf(pair._1 , pair._2 ) :: makeList(xs.tail)
      }
    }
    makeList(isort(freqs))
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.length < 2) trees
      ...
      val newElem = makeCodeTree(first, second)
      val newList = newElem :: t2
   ...
   */
  def until(fcheck: List[CodeTree] => Boolean, 
      fcombine: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = {
    if (fcheck(trees)) trees(0)
    else {
      until(fcheck, fcombine)(fcombine(trees))
    }  
  }
